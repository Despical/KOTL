...
import dev.despical.kotl.api.events.player.KOTLPlayerEnterArenaEvent;
import dev.despical.kotl.api.events.player.KOTLPlayerLeaveArenaEvent;
import dev.despical.kotl.arena.managers.BossBarManager;
import dev.despical.kotl.arena.managers.ScoreboardManager;
...
public class Arena {
    ...
    public Arena(String id) {
        ...
        if (XReflection.supports(9) && plugin.getConfigOptions().isEnabled(Option.BOSS_BAR_ENABLED)) {
    ...
    public Arena isInArea(Player player) {
        ...
        Location min = getMinCorner(), max = getMaxCorner(), origin = player.getLocation();
    ...
    public void addPlayer(Player player) {
        ...
        if (options.isEnabled(Option.INVENTORY_MANAGER_ENABLED)) {
            InventorySerializer.saveInventoryToFile(plugin, player);
        }

        if (options.isEnabled(Option.HEAL_PLAYER)) {
            AttributeUtils.healPlayer(player);
        }

        if (options.isEnabled(Option.SCOREBOARD_ENABLED)) {
            plugin.getServer().getScheduler().runTask(plugin, () -> scoreboardManager.createScoreboard(player));
        }

        if (options.isEnabled(Option.CLEAR_INVENTORY)) {
            player.getInventory().clear();
        }

        if (options.isEnabled(Option.CLEAR_EFFECTS)) {
        ...
        plugin.getServer().getScheduler().runTask(plugin, () -> {
            if (options.isEnabled(Option.UPDATE_GAME_MODE)) {
                player.setGameMode(GameMode.valueOf(plugin.getConfig().getString("Game-Mode", "SURVIVAL")));
            }

            if (options.isEnabled(Option.UPDATE_HUNGER)) {
        ...
        if (options.isEnabled(Option.JOIN_NOTIFY)) {
            plugin.getChatManager().broadcastAction(this, player, ChatManager.ActionType.JOIN);
        }

        if (options.isEnabled(Option.REMOVE_COOLDOWN_ON_JOIN)) {
            plugin.getCooldownManager().setCooldown(user, (options.isEnabled(Option.SEPARATE_COOLDOWNS) ? id : "") + "king", 0);
        }
    ...
    private void removePlayer(Player player, boolean quit) {
        ...
        if (options.isEnabled(Option.INVENTORY_MANAGER_ENABLED) && !quit) {
        ...
        if (options.isEnabled(Option.LEAVE_NOTIFY) && user.getCooldown("death") == 0) {
            plugin.getChatManager().broadcastAction(this, player, ChatManager.ActionType.LEAVE);
        }

        if (options.isEnabled(Option.REMOVE_COOLDOWN_ON_LEAVE)) {
            plugin.getCooldownManager().setCooldown(user, (options.isEnabled(Option.SEPARATE_COOLDOWNS) ? id : "") + "king", 0);
        }
